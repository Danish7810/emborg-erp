...
export default function LeadsPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setTitle(""); setValue(""); setStatus("new"); setNotes("");
    setShowForm(false);
    setSaving(false);
    fetchLeads();
  }
          ...
          {STAGES.map((stage) => (
            <div key={stage} style={{ backgroundColor: "var(--bg-alt)", borderRadius: "14px", padding: "16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
                ...
                <span style={{ fontSize: "12px", color: "var(--muted)", marginLeft: "auto" }}>{leads.filter((l) => l.status === stage).length}</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {leads.filter((l) => l.status === stage).map((lead) => (
                  <div key={lead.id} style={{ padding: "12px", backgroundColor: "var(--bg)", borderRadius: "10px", border: "1px solid var(--line)" }}>
                    ...
                    {lead.value > 0 && <p style={{ fontSize: "12px", color: "var(--accent)", margin: "0 0 8px 0", fontWeight: 600 }}>${lead.value.toLocaleString()}</p>}
