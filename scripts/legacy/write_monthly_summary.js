...
// ═══════════════════════════════════════════════════════════════════
// MONTHLY AI SUMMARY EMAIL — /api/cron/monthly-summary
// Runs on the 1st of every month at 8am IST (2:30 UTC)
// For each company: pulls last 30 days of real data, sends to Claude,
// gets a written summary, wraps in branded email, sends via Resend.
// ═══════════════════════════════════════════════════════════════════
write('app/api/cron/monthly-summary/route.ts', `import { NextRequest, NextResponse } from "next/server";
...
if (!vercel.crons) vercel.crons = [];

const alreadyHas = vercel.crons.some(c => c.path === '/api/cron/monthly-summary');
if (!alreadyHas) {
  // 1st of every month at 2:30 UTC = 8:00am IST
  vercel.crons.push({ path: '/api/cron/monthly-summary', schedule: '30 2 1 * *' });
  fs.writeFileSync(vercelPath, JSON.stringify(vercel, null, 2), 'utf8');
  console.log('✅ vercel.json: monthly summary cron added (1st of month, 8am IST)');
} else {
  console.log('⚠ vercel.json already has monthly-summary cron — skipping');
}
